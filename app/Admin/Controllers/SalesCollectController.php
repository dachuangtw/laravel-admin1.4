...
class SalesCollectController extends Controller
{
    ...
    public function view($id)
    {
        ...
        //顯示圖片欄位
        $imgArray = [];
        $salescollect['wid'] = Warehouse::find($salescollect['wid'])->name;
    ...
    protected function grid()
    {
        ...
        return Admin::grid(SalesCollect::class, function (Grid $grid) {
            ...
            $grid->filter(function($filter){
                ...
                //     $Sid_nickname = Sales::where('nickname','like',"%{$this->input}%")->pluck('id');
        ...
        return Admin::form(SalesCollect::class, function (Form $form) {
            ...
            if($url_edit) {
                ...
                // }else{
                    ...
                    ->options(Sales::all()->where('warehouse_id',Admin::user()->wid)->pluck('name','id'))->rules('required')->setWidth(2, 2);
                // }
            }
                ...
                elseif(request()->action == 'edit'){
                    ...
                    foreach(request()->pid as $key => $pid){
                        ...
                        if (isset($scdid[$key])) { //更新原本的
                            SalesCollectDetails::find($scdid[$key])->update($updateSalesCollectArray[$key]);
                            $unsetKey = array_search($scdid[$key],$deleteScdid);
                            unset($deleteScdid[$unsetKey]);
                        }elseif(empty($scdid[$key])){ //新增增加的
                            SalesCollectDetails::create($updateSalesCollectArray[$key]);
                        }
                    }
                    //刪除移除的領貨明細
